...
verus! {
        ...
        decreases i,
    {
        ...
        assert forall |k: OperationNumber|
            ...
            seq_to_set_is_finite(c.config.replica_ids);
            ...
            lemma_set_subset_finite(c.config.replica_ids.to_set(), s.unexecuted_learner_state[k].received_2b_message_senders);
        }
        ...
        if p.msg->val_2b != s_prime.unexecuted_learner_state[opn].candidate_learned_value {
            if !s.unexecuted_learner_state.contains_key(opn) {
                    ...
                    || BalLt(m->bal_2b, s.max_ballot_seen)
                {
                    ...
                    assert(s_prime.unexecuted_learner_state[opn].candidate_learned_value == p.msg->val_2b);
                    assert(false);
                } else if !s.unexecuted_learner_state.contains_key(m->opn_2b) {
                    let tup_ = LearnerTuple{
                        received_2b_message_senders: set![p.src],
                    ...
                    assert(s.unexecuted_learner_state.contains_key(m->opn_2b));
                    assert(false);
                }
            }
            assert(s.unexecuted_learner_state.contains_key(opn));
            lemma_Received2bMessageSendersAlwaysNonempty(b, c, i - 1, learner_idx, opn);
            let sender2 = choose |sender2: AbstractEndPoint|
                s.unexecuted_learner_state[opn].received_2b_message_senders.contains(sender2);
