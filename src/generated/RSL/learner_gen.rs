...
{
    s.unexecuted_learner_state.iter().filter(|(k, _)| (k >= ops_complete)).cloned().collect();
    // TODO: Value mapping - output[k] = MethodCall { receiver: Field(Var("s"), "unexecuted_learner_state"), method: "index", args: [Var("k")] }
    CLearner {
        constants: s.constants,
        max_ballot_seen: s.max_ballot_seen,
