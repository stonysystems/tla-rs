    ...
    LExecutorProcessStartingPhase2(self@, self@, inp@, result@.map(|i, p: CPacket| p@)),
{
    if contains(&self.constants.all.config.replica_ids, &inp.src) && (match &inp.msg {
        ...
        let sent_packets = crate::optimized_rsl::RSL::broadcast_gen::CBroadcastToEveryone(&self.constants.all.config, &self.constants.my_index, &CMessage::CMessageAppStateRequest {
...
pub exec fn CExecutorExecute_mut(s: &mut CExecutor) -> (sent_packets: Vec<CPacket>)
requires
    old(s).valid(),
