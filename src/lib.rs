...
verus! {
    ...
    pub unsafe extern "C" fn rsl_main_wrapper(
        ...
        match paxos_main(nc, args) {
            Ok(_) => {
                println!("Protocol exited normally");
                0
            }
            Err(_) => {
                println!("Protocol exited with error");
                1
            }
        }
    }

    /// Generic FFI entry point that dispatches to any protocol by name.
    ///
    /// `protocol_id` is a UTF-8 byte string of length `protocol_id_len` containing
    /// the protocol name (e.g., "twophase", "raft", "paxos").
    /// Returns: 0 on success, 1 on protocol error, 2 on unknown protocol.
    #[verifier(external)]
    #[verus::line_count::ignore]
    #[no_mangle]
    pub unsafe extern "C" fn protocol_main_wrapper(
        ...
        let protocol_bytes = std::slice::from_raw_parts(protocol_id, protocol_id_len as usize);
        let protocol_name = match std::str::from_utf8(protocol_bytes) {
            Ok(s) => s,
            Err(_) => {
                eprintln!("Invalid protocol name (not valid UTF-8)");
                return 2;
            }
        };
        ...
        let nc = crate::common::native::io_s::NetClient::new(EndPoint{id: my_end_point}, get_time_func, receive_func, send_func);

        let result: i32 = match protocol_name {
            "rsl" => {
                match paxos_main(nc, args) {
                    Ok(_) => 0,
                    Err(_) => 1,
            ...
            "twophase" => {
                match crate::services::TwoPhase::main_i::twophase_main(nc, args) {
                    Ok(_) => 0, Err(_) => 1,
                }
            }
            "leaderelection" => {
                match crate::services::LeaderElection::main_i::leader_election_main(nc, args) {
                    Ok(_) => 0, Err(_) => 1,
                }
            }
            "primarybackup" => {
                match crate::services::PrimaryBackup::main_i::primarybackup_main(nc, args) {
                    Ok(_) => 0, Err(_) => 1,
                }
            }
            "chainreplication" => {
                match crate::services::ChainReplication::main_i::chain_replication_main(nc, args) {
                    Ok(_) => 0, Err(_) => 1,
                }
            }
            "paxos" => {
                match crate::services::Paxos::main_i::paxos_main(nc, args) {
                    Ok(_) => 0, Err(_) => 1,
                }
            }
            "verticalpaxos" => {
                match crate::services::VerticalPaxos::main_i::vertical_paxos_main(nc, args) {
                    Ok(_) => 0, Err(_) => 1,
                }
            }
            "raft" => {
                match crate::services::Raft::main_i::raft_main(nc, args) {
                    Ok(_) => 0, Err(_) => 1,
                }
            }
            "pbft" => {
                match crate::services::PBFT::main_i::pbft_main(nc, args) {
                    Ok(_) => 0, Err(_) => 1,
                }
            }
            "epaxos" => {
                match crate::services::EPaxos::main_i::epaxos_main(nc, args) {
                    Ok(_) => 0, Err(_) => 1,
                }
            }
            _ => {
                eprintln!("Unknown protocol: {}", protocol_name);
                eprintln!("Supported: rsl, twophase, leaderelection, primarybackup, chainreplication, paxos, verticalpaxos, raft, pbft, epaxos");
                2
            }
        };

        match result {
            0 => println!("Protocol exited normally"),
            1 => println!("Protocol exited with error"),
            _ => println!("Protocol dispatch failed"),
        }
        result
    }
