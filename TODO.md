    ...
    - **31.9.4 leaf breakdown** (all leaves scoped to one lemma or one tightly-coupled pair, each expected <500 LOC touched):
      ...
      - [ ] **31.9.4.9**: `refinement_proof/requests.rs` — remove `external_body` from the remaining 3 request provenance lemmas. [2026-03-12] Scope check: de-externalizing all 3 together is likely >500 LOC due cross-lemma dependencies (`RequestIn2a` depends on request-queue provenance, and decided-request depends on `RequestIn2a`); decompose into focused leaves below.
        ...
        - [ ] **31.9.4.9.b**: Remove `#[verifier(external_body)]` from `lemma_RequestIn2aMessageHasCorrespondingRequestMessage`, using 31.9.4.9.a and existing message1b/message2a helper lemmas; pass focused check at `--rlimit 40`.
          - **Investigation (2026-03-19)**: Simply removing `external_body` causes rlimit failure even at rlimit 200 / 10min timeout. Root cause: the "old value" branch (`LProposerNominateOldValueAndSend2a`) requires unpacking a 3-level nested existential (`LValIsHighestNumberedProposal` → `LValIsHighestNumberedProposalAtBallot` → `LExistsBallotInS`) to extract a 1b witness packet. The `choose` on this chain creates a combinatorial SMT explosion. Extraction into a separate helper is blocked by mutual recursion with `lemma_RequestIn1bMessageHasCorrespondingRequestMessage` (shared `decreases i` termination argument). Next approach: add a dedicated lemma that unpacks `LProposerNominateOldValueAndSend2a` into concrete packet witnesses outside the mutual recursion, then call it from the main proof.
