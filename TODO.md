...
- 111 transpiler integration tests pass (including 10 verifying generated module public APIs, 1 verus2tla roundtrip for all 7 protocols, 10 D4 pipeline regression tests, 9 message generation per-protocol tests, 9 marshalling round-trip tests, 10 LNext scheduler analysis tests, 3 action classification tests, 9 scaffold structure tests, 9 host-init compilation tests, 14 scheduler generation tests [2 TOML roundtrip, 1 exact counts, 1 consistency, 1 heuristic coverage, 9 scaffold compilation])
...
**Active work**: Phase 17 — Runnable Protocols. Phases 17.1 (transpiler coverage), 17.2 (generic framework), 17.3.1 (message generation), 17.4.1 (LNext analysis), 17.4.2 (action classification), 17.5 (per-protocol implementations), 17.6 (C# runtime integration), and 17.7.2 marshalling round-trip tests are COMPLETE. All 10 protocols can be launched via `dotnet IronProtocolServer.dll <service> <private> protocol=<name>`. Remaining: 17.3.2-3 (RSL marshalling), 17.4.3-4 (scheduler scaffold+refinements), 17.6.3 (integration test harness), 17.7.2 (host init tests).
...
- [x] Tests for scheduler generation (17.4) — 24 tests: 2 TOML roundtrip (single+all protocols), 1 exact action counts (all 9), 1 action_count field consistency, 1 message_driven heuristic coverage, 9 scaffold compilation (compile with rustc), plus fix: scaffold field name collision with reserved names (config→msg_config)
