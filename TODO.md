...
- [x] **37.2.1.d**: Fixed Verus CI download: updated `VERUS_VERSION` to `0.2026.03.17.a96bad0` (latest stable), URL `release/rolling/` → `release/`, Rust `1.93.0` → `1.94.0`. **Remaining**: Verus compilation fails with `unresolved import vstd::std_specs::hash::KeysAdditionalSpecFns` — this API was removed in the new Verus version. Affects generated RSL code (`executor_gen.rs`, `learner_gen.rs`, `replica_gen.rs`). Fix requires updating generated code imports to use new Verus API. This is a pre-existing generated-code compatibility issue, not a CI config problem.
