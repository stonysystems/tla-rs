...
- Phase 12 (proof generation): ✅ COMPLETE — executor_gen.rs has 0 assumes (all 12 eliminated in Phase 12.2.2); 10 uniform packet-identity assumes remain in replica_gen.rs (irreducible IO trust boundary); 1288 transpiler tests pass
...
- 121 transpiler integration tests pass (including 10 verifying generated module public APIs, 1 verus2tla roundtrip for all 7 protocols, 10 D4 pipeline regression tests, 9 message generation per-protocol tests, 9 marshalling round-trip tests, 10 LNext scheduler analysis tests, 3 action classification tests, 9 scaffold structure tests, 9 host-init compilation tests, 15 scheduler generation tests [2 TOML roundtrip, 1 exact counts, 1 consistency, 1 message_variant validity, 1 heuristic coverage, 9 scaffold compilation])
...
1. ~~**Phase 17: Runnable protocols**~~ ✅ MOSTLY COMPLETE — All 9 non-RSL protocols have runnable implementations with networking, marshalling, and main loop. Remaining: 17.3.2/17.3.3 (RSL-specific marshalling, deferred), 17.6.3 (cluster integration tests, requires .NET SDK).
...
**Active work**: Phase 17 — Runnable Protocols. All sub-phases COMPLETE except: 17.3.2-3 (RSL marshalling — deferred, RSL-specific), 17.6.3 (integration test harness — blocked, requires .NET SDK). All 10 protocols can be launched via `dotnet IronProtocolServer.dll <service> <private> protocol=<name>`. 1288 total tests (976 unit + 122 integration + 53 tla_examples + 43 roundtrip + 38 roundtrip_test + 19 regression + 14 negative + 12 pipeline_e2e + 11 main), 0 warnings.
...
- [ ] Once transpiler output is verified, replace `src/implementation/RSL/` with generated code (wiring complete via clone-delegate pattern; full replacement requires making generated modules standalone without delegating to manual impl)
