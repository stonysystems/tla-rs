...
- Phase 12 (proof generation): ✅ COMPLETE — executor_gen.rs has 0 assumes (all 12 eliminated in Phase 12.2.2); 10 uniform packet-identity assumes remain in replica_gen.rs (irreducible IO trust boundary); 1287 transpiler tests pass
...
**Active work**: Phase 17 — Runnable Protocols. All sub-phases COMPLETE except: 17.3.2-3 (RSL marshalling — deferred, RSL-specific), 17.6.3 (integration test harness — blocked, requires .NET SDK). All 10 protocols can be launched via `dotnet IronProtocolServer.dll <service> <private> protocol=<name>`. 1287 total tests (976 unit + 121 integration + 53 tla_examples + 43 roundtrip + 38 roundtrip_test + 19 regression + 14 negative + 12 pipeline_e2e + 11 main), 0 warnings.
