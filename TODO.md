...
- [ ] **34.7.1**: Prove `LeaderCompleteness(ds_)` inductive.  
  ...
  - [ ] **34.7.1.e.3**: Integrate overlap + log-up-to-date bridges into the new-leader branch, including the missing VoteResponse→RequestVote provenance hook (or a supporting invariant if required).
    ...
    - [ ] **34.7.1.e.3.b**: Add extraction helper(s) that turn the provenance invariant witness into explicit RequestVote parameters for overlap voter `w` in the new-leader branch.
    - [ ] **34.7.1.e.3.c**: Wire the extracted provenance witness into `lemma_leader_completeness_inductive` new-leader reasoning and connect it to the existing log-up-to-date bridge helpers.
