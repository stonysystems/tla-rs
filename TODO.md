...
✅ **580 verified, 0 errors** (with 10 uniform packet-identity assumes in replica_gen.rs).
...
- Phase 12 (proof generation): ✅ COMPLETE — executor_gen.rs has 0 assumes (all 12 eliminated in Phase 12.2.2); 10 uniform packet-identity assumes remain in replica_gen.rs (irreducible IO trust boundary); 907 transpiler tests pass
...
- Full codebase: 580 verified, 0 errors (RSL + TwoPhase + Paxos + LeaderElection + Raft + ChainReplication + PrimaryBackup + PBFT + VerticalPaxos + EPaxos)
...
- 50 transpiler integration tests pass (including 10 verifying generated module public APIs, 1 verus2tla roundtrip for all 7 protocols, 10 D4 pipeline regression tests)
...
1. **Phase 17: Runnable protocols** — Generate complete runnable implementations for all 9 non-RSL protocols with networking, marshalling, and main loop. See Phase 17 section below.
2. ~~**Phase 12: Generate proof code to eliminate assumes**~~ ✅ Phase 12.2.2 COMPLETE (12 executor assumes eliminated; 10 IO trust boundary assumes are irreducible). Phase 12.2.7 unreachable arms DONE. Remaining deferred items (12.2.3-12.2.6) are low priority.
3. ~~**Phase 16: End-to-end compile & run testing**~~ ✅ COMPLETE — all 4 directions × all examples compile and verify; status matrix complete
4. ~~**Phase 13: Port `tla+2tlars` branch features to main**~~ ✅ COMPLETE
5. ~~Eliminate remaining IO trust boundary assumes~~ 10 uniform packet-identity assumes — irreducible without IO architecture change. See `docs/dev/io-trust-boundary-analysis.md`.
5. ~~Phase 14: Regeneration audit~~ ✅ DONE
6. ~~Write a doc explaining how to check/test whether current TLA+ -> Verus and Verus -> TLA+ conversions work correctly~~ ✅ DONE — see `docs/conversion-testing-guide.md`

**Active work**: Phase 17 — Runnable Protocols. Goal: make all 9 non-RSL protocols (TwoPhase, Paxos, LeaderElection, Raft, ChainReplication, PrimaryBackup, PBFT, VerticalPaxos, EPaxos) generate complete runnable implementations with networking, marshalling, and main loop. Core protocol logic is transpiler-generated from specs and Verus-verified. The 10 packet-identity assumes in replica_gen.rs are the irreducible IO trust boundary. Phase 12 deferred items (12.2.3-12.2.6) are low priority since they're automatic for simple protocols.
...
| ChainReplication | 10 | 8 | LReconfigure | 89% |
| Paxos | 9 | 6 | LSend2a, LLearn | 78% |
| Raft | 13 | 8 | LBecomeLeader, LFollowerAppendEntries, LHandleAppendResponse, LHandleAppendReject | 67% |
| PBFT | 11 | 8 | LEnterCommit, LExecuteReply | 82% |
| VerticalPaxos | 12 | 10 | LCommit | 91% |
| EPaxos | 13 | 9 | LFastCommit, LStartAccept, LSlowCommit | 75% |
...
- [ ] Generate `let mut __log = clone_log(&s.log); if cond { __log.push(entry); }` in exec code
...
- [ ] Add transpiler tests for existential quorum codegen

**17.1.5: Support reconfiguration patterns** — Unblocks: ChainReplication/LReconfigure, VerticalPaxos/LCommit
- [ ] Parse spec functions with complex multi-field conditional updates (role reassignment)
- [ ] Generate exec code for chain role recalculation (Head/Middle/Tail based on position)
- [ ] Handle VerticalPaxos LCommit quorum check (similar to 17.1.4)
- [ ] Add transpiler tests

**17.1.6: Regenerate all protocols and verify 100% coverage**
- [ ] Remove all entries from skip_functions except LNext in every *_transpile.toml
- [ ] Run transpiler on all 9 protocols, verify no errors
- [ ] Run `scons --verus-path=...` to verify all generated code passes Verus
- [ ] Update coverage matrix: all protocols should show 100% (except LNext)
...
Extend the transpiler to auto-generate `Marshalable` implementations from type definitions, so marshalling code isn't hand-written per protocol.

**17.3.1: Generate `Marshalable` for enum types (CMessage variants)**
- [ ] Parse `CMessage` enum from `types_gen.rs` or `types.rs`
- [ ] Generate tag-based serialization: each variant gets a unique u64 tag
- [ ] Generate field-by-field serialization using existing Marshalable impls for primitives
- [ ] Generate `deserialize()` with tag dispatch: read tag → match → deserialize fields
- [ ] Handle nested types: EndPoint, Vec<u8>, HashMap keys/values
...
**17.3.3: Add `generate-marshalling` transpiler subcommand**
- [ ] New CLI: `verus-transpile generate-marshalling --types types_gen.rs --output messages.rs`
- [ ] Reads type definitions, emits `impl Marshalable for CMessage { ... }` etc.
- [ ] Include Verus proof annotations (ghost_serialize, is_marshalable)
- [ ] Add transpiler tests for marshalling generation
...
**17.4.1: Parse LNext disjunction structure**
- [ ] Parse `LNext(s, s_, c)` spec body to extract list of action predicates
- [ ] For each disjunction branch, record: action name, existential parameters, guard conditions
- [ ] Example: `LNext = exists|voter| LReceiveVoteGranted(s,s_,c,voter) || LTimeout(s,s_,c) || ...`

**17.4.2: Generate action dispatch enum and scheduler**
- [ ] Generate `enum ProtocolAction { Timeout, GrantVote, ReceiveVote, ... }` from LNext branches
- [ ] Generate round-robin scheduler: `fn next_action(index: usize) -> ProtocolAction`
- [ ] Generate dispatch: `match action { Timeout => CTimeout(s, c), ... }`
- [ ] Handle existential parameters: extract from received message or environment

**17.4.3: Generate `HostState::next_impl()` for each protocol**
- [ ] Combine: receive_packet → determine applicable action → call C-function → deliver outbound
- [ ] Generate message-driven dispatch: incoming message variant → matching action
- [ ] Generate timer-driven actions: timeouts, heartbeats
- [ ] Output: `src/implementation/P/host.rs` (generated, not hand-written)
...
**17.6.1: Parameterize C# entry point by protocol**
- [ ] Add protocol selection to C# Program.cs (command-line arg: `--protocol raft|twophase|...`)
- [ ] Each protocol shares the same UDP transport (IoNative.cs — unchanged)
- [ ] Each protocol gets its own FFI entry: `raft_main_wrapper`, `twophase_main_wrapper`, etc.

**17.6.2: Build system updates**
- [ ] Update SConstruct to build per-protocol service binaries
- [ ] `scons bin/IronTwoPhaseServer.dll`, `scons bin/IronRaftServer.dll`, etc.
- [ ] Or single binary with `--protocol` flag
