...
✅ **570 verified, 0 errors** (with 10 uniform packet-identity assumes in replica_gen.rs + 16 assume(false) in generated RSL code). Count dropped from 585 because assume(false) functions that were proven are now counted differently by Verus.
...
- [ ] No RSL function has `assume(false)` without a real exec body beside it *(16 remaining — Tier B blocked by union_sets, transpiler bugs, complex View mappings, missing external_body ensures)*
