...
- [x] **37.2.1.d**: Fixed Verus CI download: updated `VERUS_VERSION` from `0.2026.02.03.6d23bed` (non-existent release, wget exit code 8) to `0.2026.03.17.a96bad0` (latest stable). Changed download URL from `release/rolling/` to `release/`. Updated Rust toolchain `1.93.0` → `1.94.0` (required by new Verus). Note: Verus verification of the codebase may still hit rlimit issues — those are pre-existing.
