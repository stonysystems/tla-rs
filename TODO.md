...
✅ **587 verified, 0 errors** (~74 assumes remaining, was ~244). Phase 12.1 + 12.4 + 12.5.1-12.5.6 complete — 170 assumes eliminated (TwoPhase + LeaderElection + Paxos + ChainReplication + Raft + RSL/broadcast + RSL/learner + RSL/executor + RSL/acceptor + RSL/election + RSL/proposer).
Types are generated by the transpiler; 4 marshalable types (CBallot, CRequest, CReply, CVote) remain in types_i.rs for `define_struct_and_derive_marshalable!` macro compatibility.
...
- Phase 11 (type generation): ✅ Complete — all types generated in `types_gen.rs`, impl files deduplicated, configuration/constants/parameters subsumed
- Phase 12 (proof generation): ❌ TODO — ~236 `assume()` calls to eliminate across 11 generated files (TwoPhase done)
- Phase 13 (port tla+2tlars): ❌ TODO — verus2tla module (~2.8K LOC), roundtrip module (~1.2K LOC), TLA+ specs, docs
- Full codebase: 579 verified, 0 errors (RSL + TwoPhase + Paxos + LeaderElection + Raft + ChainReplication)
- Protocol examples: RSL, TwoPhase, Single-Decree Paxos, Bully Leader Election, Raft Consensus, Chain Replication

**What doesn't work yet:**
- Phase 12 (proof generation): Generated exec code uses ~244 `assume()` calls instead of real proofs
- Phase 5 (replace manual implementation): Blocked by deferred optimized variants
- TLA+ end-to-end pipeline: `--tla-input` flag not yet implemented

**Next steps (priority order):**
1. **Phase 12: Generate proof code to eliminate assumes** — TOP PRIORITY
2. **Phase 13: Port `tla+2tlars` branch features to main** (verus2tla, roundtrip, TLA+ specs)
3. Additional protocol targets: Primary-Backup, PBFT, etc.
4. Phase 5: Replace manual implementation (when optimized variants are ready)
5. TLA+ end-to-end pipeline
...
The generated exec code passes Verus verification but relies on **~244 `assume()` calls** instead of real proofs. These assumes are essentially "trust me" annotations that bypass verification — the code compiles and Verus reports 0 errors, but the correctness guarantees are hollow. The goal is to replace these assumes with actual proof code so Verus can verify that the exec implementation truly refines the spec.
...
| `RSL/proposer_gen.rs` | ~~31~~ 0 ✅ | Proposer ballot/proposal logic |
| `RSL/acceptor_gen.rs` | 23 | Acceptor vote management |
| `RSL/election_gen.rs` | ~~22~~ 0 ✅ | Election state machine |
...
The assumes fall into 6 categories, each requiring a different proof strategy:

#### Category 1: Validity Assumes (~65, 27%)
```rust
assume(result.valid());
```
**Why it's there:** After constructing a new struct, we need to prove all fields satisfy the `valid()` predicate (e.g., bounds, lengths, non-empty constraints). Currently assumed.
**Proof strategy:** Generate field-by-field assertions that propagate input validity to output validity. The `valid()` predicate is a conjunction of per-field conditions — prove each conjunct from the `requires` clause and construction logic.

#### Category 2: Spec Refinement Assumes (~72, 30%)
```rust
assume(LSpecPredicate(s@, result@, inp@, ...));
```
**Why it's there:** The core correctness property — the exec code must refine the spec predicate. Currently assumed rather than proved.
**Proof strategy:** The spec predicates are conjunctions of field equalities and simple conditions. Generate `assert` statements for each conjunct, leveraging the View trait (`@`) to relate concrete fields to spec fields. Most conjuncts reduce to `result.field@ == expr@` which follows directly from construction.

#### Category 3: Precondition Assumes (~42, 17%)
```rust
assume(msg.valid());          // before using msg
assume(new_vote.valid());     // before passing to function
assume(forall |p| ...);       // quantifier precondition
```
**Why it's there:** When calling a helper function that has `requires x.valid()`, we need to prove the argument is valid. Currently assumed.
**Proof strategy:** Track validity through the function — if input is valid and construction preserves validity, intermediate values are valid. Generate validity assertions after each construction.

#### Category 4: HashMap/HashSet Loop Assumes (~10, 4%)
```rust
// In HashMap filter loops:
assume(votes@.contains_key(*opn));                    // iterator yields keys from the map
assume((votes_keys@.0 == votes_keys@.1.len()));       // iterator has visited all keys
assume((seen_keys.len() == votes_keys@.0));            // ghost tracking matches
```
**Why it's there:** Verus's HashMap iterator spec is incomplete — `for opn in iter:votes_keys` doesn't provide strong enough invariants about key membership. The `votes.keys()` iterator spec uses a tuple `(index, seq)` but the connection between the sequence and the map's domain is not fully proven.
**Proof strategy:** This is the hardest category. Options:
- (a) Extend Verus helper lemmas for HashMap iteration (prove iterator yields domain elements)
- (b) Convert to `while` loops over `hashmap_to_vec()` (similar to `hashset_to_vec()` pattern)
- (c) Keep as trusted external_body helper with proven interface
...
**Proof strategy:** Replace `assume(false)` with `proof { /* assert from requires */ }` — the requires clause already guarantees which variant we're in. Generate the appropriate assertion from the function's precondition.

#### Category 6: Arithmetic Overflow Assumes (~1, <1%)
```rust
assume(b.seqno < u64::MAX);
```
**Why it's there:** Verus checks overflow for `u64` arithmetic.
**Proof strategy:** Propagate bounds from `valid()` predicates or add explicit bounds to the `requires` clause.

### Implementation Plan

#### Phase 12.1: Start with Simple Protocols (TwoPhase, LeaderElection, Paxos) — 8+10+10 = 28 assumes

These protocols have simple struct construction with no HashMap iteration or complex nested types. They are ideal for developing and testing the proof generation approach.

**12.1.1: Manual proof for TwoPhase (8 assumes)** ✅ Done
- [x] Manually write proof code (assert chains) to replace all 8 assumes in `twophase_gen.rs`
- [x] Document the proof patterns used: `lemma_empty_set_map()` for init, `broadcast use Set::lemma_set_map_insert_commute` for insert, added `s.tm_state is Init` preconditions
- [x] Run Verus — 0 assumes remaining in TwoPhase

**12.1.2: Manual proof for LeaderElection (10 assumes)** ✅ Done
- [x] Manually write proof code to replace all 10 assumes in `election_gen.rs`
- [x] Added `lemma_set_map_remove_commute()` for HashSet remove proofs (new Pattern 6)
- [x] Added input preconditions: `s@.alive.contains(...)`, `s@.electing.contains(...)`, `!s.has_highest || ...`
- [x] Run Verus — 582 verified, 0 errors, 0 assumes remaining in LeaderElection

**12.1.3: Manual proof for Paxos (10 assumes)** ✅ Done
- [x] Manually write proof code to replace all 10 assumes in `paxos_gen.rs`
- [x] No Option types needed — simplified Paxos uses set-based representation
- [x] Used same patterns: `lemma_empty_set_map()` for init, `broadcast use Set::lemma_set_map_insert_commute` for Send1b/Send2b
- [x] Run Verus — 583 verified, 0 errors, 0 assumes remaining in Paxos

**12.1.4: Extract common proof patterns**
- [ ] Catalog all patterns used in 12.1.1-12.1.3:
  - Struct construction refinement (field-by-field `assert`)
  - Validity propagation (input valid → output valid)
  - Enum variant proof (requires says variant A → match arm A is taken)
  - Collection operation refinement (HashSet insert/remove, Vec push)

#### Phase 12.2: Extend Transpiler for Proof Generation (~300 LOC)

Based on patterns discovered in 12.1, add proof generation to the transpiler.

**12.2.1: Add proof annotation format to .automan files**
- [ ] Design annotation syntax for proof hints:
  ```
  module Protocol::component {
      LInit(-, +);
      @proof LInit: field_equality;    // proof strategy hint
  }
  ```
- [ ] Implement parser for proof annotations in `annotation/mod.rs`

**12.2.2: Generate validity proofs (Category 1)**
- [ ] In `translator/mod.rs`, add `generate_validity_proof()` function
- [ ] For each field of the constructed result, generate an assertion that the field satisfies its validity condition
- [ ] Handle primitive fields (u64 bounds), collection fields (len checks), nested struct fields (recursive valid())
- [ ] Config option: `generate_proofs = true` (default false, for backward compatibility)

**12.2.3: Generate spec refinement proofs (Category 2)**
- [ ] In `translator/mod.rs`, add `generate_refinement_proof()` function
- [ ] Parse the spec predicate body to extract individual conjuncts
- [ ] For each conjunct, generate an `assert` that the exec construction satisfies it
- [ ] Handle `=~=` (extensional equality), `==` (structural equality), field projections
- [ ] Handle conditional branches: generate proofs for both if/else arms

**12.2.4: Generate precondition proofs (Category 3)**
- [ ] Track validity state through the function body
- [ ] After each struct construction or function call, assert validity of the result
- [ ] Before each function call, assert the callee's requires clause is satisfied
- [ ] Use the caller's requires + intermediate assertions as proof context

**12.2.5: Handle unreachable arms (Category 5)**
- [ ] Replace `assume(false); unreachable_value()` with `proof { assert(false) by { /* from requires */ } }; unreachable_value()`
- [ ] Extract the relevant requires clause and generate the contradiction proof

#### Phase 12.3: Tackle HashMap/HashSet Loops (Category 4) (~200 LOC)

This is the hardest part — HashMap iteration proofs in Verus.

**12.3.1: Analyze existing loop patterns**
- [ ] Identify all HashMap filter loops in generated code (acceptor_gen.rs: 2 loops, others)
- [ ] Identify all HashSet iteration loops (election_gen.rs, proposer_gen.rs)
- [ ] Document what each loop needs to prove (filter semantics, membership, completeness)

**12.3.2: Develop HashMap iteration proof strategy**
- [ ] Option A: `hashmap_to_vec()` conversion (like existing `hashset_to_vec()`)
  - Add `hashmap_keys_to_vec()` or `hashmap_to_entries_vec()` helper
  - Convert `for opn in iter:votes_keys` to `while` loop over vec
  - Prove: vec contains exactly the map's domain elements
- [ ] Option B: Strengthen loop invariants for `for ... in iter:` pattern
  - Add `seen_keys` ghost tracking with proper subset/superset assertions
  - Prove iterator completeness after loop via `subset_len_equal_implies_equal`
- [ ] Option C: `#[verifier(external_body)]` wrapper that provides a proven interface
  - Create `hashmap_filter()` helper with verified ensures clause
  - Use it instead of inline loops

**12.3.3: Implement chosen strategy**
- [ ] Implement helper functions (e.g., `hashmap_filter()`) in `common/collections/`
- [ ] Update transpiler to generate calls to these helpers instead of raw loops
- [ ] Verify helpers pass Verus independently

**12.3.4: Eliminate loop assumes**
- [ ] Replace all HashMap loop assumes with actual proofs or verified helper calls
- [ ] Run Verus — target: 0 loop-related assumes

#### Phase 12.4: Apply to Raft and ChainReplication (20+14 = 34 assumes)

**12.4.1: Generate proofs for ChainReplication (14 assumes)** ✅ Done
- [x] Replace all 14 assumes in `chain_gen.rs` with actual proofs
- [x] Added Seq push-map commutativity lemma (Pattern 7), empty seq map lemma
- [x] Added `clone_role()` helper for CNodeRole (workaround for missing derive Clone spec)
- [x] Added `ensures` to CState::clone() for CClientRead identity proof
- [x] Fixed CInit precondition: `c.chain_len >= 2` (not 1 — spec requires distinct head/tail)
- [x] Run Verus — 588 verified, 0 errors, 0 assumes remaining in ChainReplication

**12.4.2: Generate proofs for Raft (20 assumes)** ✅ Done
- [x] Apply transpiler proof generation to `raft_gen.rs`
- [x] Handle Raft-specific patterns (log clone view, vote tracking, HashMap insert)
- [x] Run Verus — 592 verified, 0 errors, 0 assumes remaining in Raft

#### Phase 12.5: Apply to RSL (182 assumes — the main target)

RSL is the most complex protocol. Apply proof generation systematically by component.

**12.5.1: broadcast_gen.rs (2 assumes)** ✅ Done
- [x] Prove CBroadcastToEveryone refinement with loop invariant
- [x] Added `*myidx < c.replica_ids.len()` as requires precondition
- [x] Used `clone_up_to_view()` for view-preserving clones in loop body
...
**12.5.3: executor_gen.rs (~~19~~ 0 assumes)** ✅ COMPLETE — 595 verified, 0 errors
- [x] Prove CExecutorInit (empty reply cache lemma, field-level validity)
- [x] Prove CExecutorGetDecision (clone_up_to_view for constants/reply_cache/batch)
- [x] Prove CClientsInReplies (delegate to verified CExecutor::CClientsInReplies)
- [x] Prove CExecutorExecute (CHandleRequestBatch property lemmas, RepliesAreReplyType, HandleRequestBatch spec-level length)
- [x] Prove CExecutorProcessAppStateSupply (message destructuring, validity from inp.valid())
- [x] Prove CExecutorProcessAppStateRequest (2-branch, packet construction, clone_up_to_view)
- [x] Prove CExecutorProcessStartingPhase2 (CBroadcastToEveryone ensures, clone_up_to_view)
- [x] Prove CExecutorProcessRequest (lemma_creplycache_get for HashMap→abstractify connection)
- Key patterns: delegation to verified impl, abstractify_creplycache lemmas, CHandleRequestBatch property lemmas, RepliesAreReplyType induction lemma

**12.5.4: acceptor_gen.rs (23 assumes)** ✅ DONE (592 verified, 0 errors, ~126 assumes)
- [x] Added CAcceptor.clone_up_to_view() to types_gen.rs
- [x] Added clone_cpacket_preserving_validity() helper — CPacket clone that preserves validity
- [x] Added outbound_packets_to_vec() helper — converts OutboundPackets to Vec<CPacket>
- [x] CRemoveVotesBeforeLogTruncationPoint: Delegate to CAcceptor::CRemoveVotesBeforeLogTruncationPoint (5 assumes eliminated)
- [x] CAddVoteAndRemoveOldOnes: Delegate to CAcceptor::CAddVoteAndRemoveOldOnes (6 assumes eliminated)
- [x] CAcceptorInit: Delegate to CAcceptor::CAcceptorInit (2 assumes eliminated)
- [x] CAcceptorProcess1a: Clone-delegate-extract via outbound_packets_to_vec (2 assumes eliminated)
- [x] CAcceptorProcess2a: Clone-delegate + outbound_packets_to_vec (5 assumes eliminated)
- [x] CAcceptorProcessHeartbeat: Clone-delegate pattern (2 assumes eliminated)
- [x] CAcceptorTruncateLog: Clone-delegate pattern (2 assumes eliminated)
- Key patterns: delegation to verified impl (Pattern 15), clone-delegate-extract (Pattern 19), OutboundPackets→Vec conversion (Pattern 20), CPacket validity clone (Pattern 21)

**12.5.5: election_gen.rs (22 assumes)** ✅ Done
- [x] CComputeSuccessorView: Delegate to CElectionState::CComputeSuccessorView (3 assumes eliminated)
- [x] CBoundRequestSequence: Delegate to CElectionState::CBoundRequestSequence (1 assume eliminated)
- [x] CRequestsMatch: Delegate to CElectionState::CRequestsMatch (1 assume eliminated)
- [x] CRequestSatisfiedBy: Delegate to CElectionState::CRequestSatisfiedBy (1 assume eliminated)
- [x] CRemoveAllSatisfiedRequestsInSequence: Delegate to CElectionState::CRemoveAllSatisfiedRequestsInSequence (2 assumes eliminated)
- [x] CElectionStateInit: Delegate to CElectionState::CElectionStateInit (2 assumes eliminated)
- [x] CElectionStateProcessHeartbeat: Clone-delegate pattern (2 assumes eliminated)
- [x] CElectionStateCheckForViewTimeout: Clone-delegate pattern (2 assumes eliminated)
- [x] CElectionStateCheckForQuorumOfViewSuspicions: Clone-delegate pattern (2 assumes eliminated)
- [x] CElectionStateReflectReceivedRequest: Clone-delegate pattern (4 assumes eliminated)
- [x] CRemoveExecutedRequestBatch: Delegate to CElectionState::CRemoveExecutedRequestBatch (2 assumes eliminated)
- Key patterns: delegation to verified impl, clone-delegate for &mut self methods, added preconditions for validity propagation

**12.5.6: proposer_gen.rs (31 assumes)** ✅ Done
- [x] Added CProposer::clone_up_to_view() to types_gen.rs (ensures self==result, view, valid preservation)
- [x] CProposerInit: Delegate to CProposer::CProposerInit (2 assumes eliminated)
- [x] CProposerProcessRequest: Clone-delegate pattern (4 assumes eliminated)
- [x] CProposerMaybeEnterNewViewAndSend1a: Clone-delegate + outbound_packets_to_vec (2 assumes eliminated)
- [x] CProposerProcess1b: Clone-delegate + clone_cpacket_full for concrete equality (2 assumes eliminated)
- [x] CProposerMaybeEnterPhase2: Clone-delegate + outbound_packets_to_vec (2 assumes eliminated)
- [x] CProposerNominateNewValueAndSend2a: Clone-delegate + outbound_packets_to_vec (2 assumes eliminated)
- [x] CProposerNominateOldValueAndSend2a: Clone-delegate + outbound_packets_to_vec (5 assumes eliminated)
- [x] CProposerMaybeNominateValueAndSend2a: Clone-delegate + outbound_packets_to_vec (2 assumes eliminated)
- [x] CProposerProcessHeartbeat: Clone-delegate pattern (2 assumes eliminated)
- [x] CProposerCheckForViewTimeout: Clone-delegate pattern (2 assumes eliminated)
- [x] CProposerCheckForQuorumOfViewSuspicions: Clone-delegate pattern (2 assumes eliminated)
- [x] CProposerResetViewTimerDueToExecution: Clone-delegate + added crequestbatch_is_valid requires (4 assumes eliminated)
- Key patterns: clone_cpacket_full for concrete EndPoint equality, abstract src@ inequality for 1b uniqueness
- Note: Added 1 assume to replica_gen.rs caller (crequestbatch_is_valid for CProposerResetViewTimerDueToExecution)

**12.5.7: replica_gen.rs (73 assumes)**
- [ ] Prove CReplicaInit, all CReplicaNext* dispatch functions
- [ ] Most assumes are at call sites — once components are proved, many fall away
- [ ] Handle IO dispatch proofs (matching ios to sent_packets)
- [ ] Handle unreachable match arms (6 assumes)

#### Phase 12.6: Verification and Cleanup

**12.6.1: Run full Verus verification**
- [ ] Target: 579+ verified, 0 errors, **0 assumes** in generated code
- [ ] All proofs should be Verus-checked, not trusted

**12.6.2: Audit remaining assumes (if any)**
- [ ] Document any assumes that cannot be eliminated and why
- [ ] For genuinely unprovable properties, convert to `#[verifier(external_body)]` helpers with clear documentation
- [ ] Ensure all remaining trusted code is minimal and well-justified

**12.6.3: Update transpiler tests**
- [ ] Add tests for proof generation: `cargo test --lib`
- [ ] Test that proof generation works for each category
- [ ] Regression tests: regenerate all protocols and verify

**12.6.4: Update scripts and documentation**
- [ ] Update `scripts/regenerate_rsl.sh` for proof generation options
...
2. **View trait reasoning**: Proofs require reasoning through the View trait — `result.field@ == spec_expr` must be proved by showing the concrete construction maps correctly through `@`.

3. **HashMap iteration completeness**: The hardest problem. Proving that a `for` loop over HashMap keys visits exactly the domain requires lemmas not currently in Verus's standard library.

4. **Quantifier instantiation**: Some spec predicates use `forall` quantifiers. The exec proof needs to provide witnesses or trigger instantiation for Verus's SMT solver.

5. **Recursive helper functions**: Functions like `CClientsInReplies` are recursive and need inductive proofs — the transpiler must generate decreases clauses and inductive assertions.

### Success Criteria

1. [ ] **0 `assume()` calls** in all generated files under `src/generated/`
2. [ ] Verus: **579+ verified, 0 errors** (same or better than current)
3. [ ] All proofs are machine-checked by Verus (no `#[verifier(external_body)]` on proof code)
4. [ ] Transpiler can regenerate all protocols with proofs: `cargo run -- -i spec.rs ... --proofs`
5. [ ] `cd transpiler && cargo test --lib`: all tests pass

### Estimated Effort

| Phase | Description | Assumes Eliminated | Status |
|-------|-------------|-------------------|--------|
| 12.1 | Manual proofs for simple protocols | 28 | ✅ 28/28 done |
| 12.2 | Transpiler proof generation | 0 (infrastructure) | ❌ TODO |
| 12.3 | HashMap/HashSet loop proofs | ~10 | ❌ TODO |
| 12.4 | Raft + ChainReplication proofs | 34 | ✅ 34/34 done |
| 12.5 | RSL proofs (all components) | 182 | ❌ TODO |
| 12.6 | Verification + cleanup | 0 (validation) | ❌ TODO |
| **Total** | | **~244** | |
...
**13.1.1: Copy verus2tla source files**
- [ ] Create `transpiler/src/verus2tla/` directory on main
- [ ] Port `mod.rs`, `converter.rs`, `printer.rs`, `types.rs` from `tla+2tlars`
- [ ] Adapt imports to match main's current module structure
- [ ] Ensure `cargo check` passes

**13.1.2: Add verus2tla CLI subcommand**
- [ ] Add `Verus2Tla` variant to `Commands` enum in `main.rs`
- [ ] Add single-file and `--batch` mode support
- [ ] Wire up CLI to verus2tla converter

**13.1.3: Register module in lib.rs**
- [ ] Add `pub mod verus2tla;` to `transpiler/src/lib.rs`
- [ ] Add re-exports as needed

**13.1.4: Port verus2tla tests**
- [ ] Port unit tests from verus2tla module (26 tests)
- [ ] Port integration tests from `transpiler/tests/integration.rs` (verus2tla-related tests)
...
- [ ] Generate TLA+ for TwoPhase, Paxos, LeaderElection, Raft, ChainReplication
- [ ] These protocols only exist on main, so this is new work (not a port)

#### Phase 13.4: Port Documentation

**13.4.1: Copy documentation files**
- [ ] Port `docs/dev/verus2tla-design.md`
- [ ] Port `docs/dev/phase2_roundtrip_design.md`
- [ ] Port `docs/tla_features.md`
- [ ] Port `docs/verus_features.md`
- [ ] Port `docs/migration_guide.md`

#### Phase 13.5: Verify and Clean Up

**13.5.1: Full test suite**
- [ ] `cd transpiler && cargo test --lib` — all tests pass
- [ ] `cd transpiler && cargo test` — all integration tests pass
- [ ] Verus verification: 580+ verified, 0 errors

**13.5.2: Delete branch**
- [ ] Confirm all tla+2tlars features are on main
- [ ] Delete `origin/tla+2tlars` branch

### Success Criteria

1. [ ] `verus2tla` CLI subcommand works on main (single-file and batch mode)
2. [ ] Roundtrip consistency tests pass (25 tests)
3. [ ] `src/tla+/RSL/*.tla` generated and valid
4. [ ] All transpiler tests pass (`cargo test`)
5. [ ] Verus verification still passes (580+ verified, 0 errors)
6. [ ] `tla+2tlars` branch can be safely deleted

### Estimated Effort

| Phase | Description | LOC | Status |
|-------|-------------|-----|--------|
| 13.1 | Port verus2tla module + CLI | ~2,948 | ❌ TODO |
| 13.2 | Port roundtrip module + tests | ~1,586 | ❌ TODO |
| 13.3 | Generate TLA+ specs | ~700+ | ❌ TODO |
| 13.4 | Port documentation | ~1,122 | ❌ TODO |
| 13.5 | Verify + delete branch | ~0 | ❌ TODO |
| **Total** | | **~6,356** | |
