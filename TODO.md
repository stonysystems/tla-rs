      ...
      - [ ] **34.7.1.e.4.b.2**: In the unchanged-leader + fresh-step path, discharge the remaining subcase `!commit_quorum.contains(leader_id)` without `assume`, using overlap with leader-election quorum / log relation as needed.
        ...
        - [ ] **34.7.1.e.4.b.2.b**: Use the constructed overlap witness plus existing vote/provenance/log-relation bridges to prove the unchanged leader has `entry` at `k`, removing the remaining local `assume` in this subcase.
          ...
          - [ ] **34.7.1.e.4.b.2.b.3**: Remove the remaining 3 `assume(false)` in the strict-term branches of `lemma_overlap_voter_entry_transfer` (lines 2694, 2718, 2732). All 3 hit the `d_rli ≤ k` wall: in strict-term case (`rlt > vtl`), ETHVQ vote dests at different terms can't use `vote_dest_unique`, and LogMatching has no anchor at or above k. These are in the fresh-commit path (`!EntryCommittedAt(ds, k, entry)`), limiting available tools. Decomposed below.
            - [ ] **34.7.1.e.4.b.2.b.3.a**: **Analysis**: The 3 strict-term assumes in `lemma_overlap_voter_entry_transfer` share the same structure: `leader.log[k].term != entry.term` in cases where LogMatching can't bridge. Options: (A) Leader-term strong induction — restructure proof to descend on leader terms instead of ETHVQ vote dest terms, using LC(ds) from IH. Challenge: ETHVQ vote dest at term T' may not be a current Leader in ds, so LC(ds) can't be applied directly. (B) Prove these cases unreachable by showing `leader.log[k].term == entry.term` must hold (using near-quorum + LogTermsMonotonic + log_up_to_date constraints). (C) Add ghost provenance to log entries (spec-level refactoring). Recommended: try (B) first as it may not require restructuring.
            - [ ] **34.7.1.e.4.b.2.b.3.b**: Resolve assume at line 2694 (strict-term, rli>L, `leader.log[L-1].term != vtl`, `leader.log[k].term != entry.term`). In this case: `rlt > vtl`, `rli > L > k`, `leader.log.len() > k`. The leader has entries up to at least rli-1 > L-1 > k. `leader.log[k].term` is some value between 0 and rlt. If `leader.log[k].term > entry.term`: ETHVQ at `(leader, k, leader.log[k].term)` gives a vote dest with quorum at that term; overlap with commit quorum gives an overlap voter with entry at k. The overlap voter's log_up_to_date constraints may force a contradiction via LogTermsMonotonic chains. If `leader.log[k].term < entry.term`: entry.term <= vtl (by LogTermsMonotonic on voter, k < L), and `leader.log[k].term < entry.term`. Entry is at a higher term than leader's entry at k. May need the near-quorum argument.
            - [ ] **34.7.1.e.4.b.2.b.3.c**: Resolve assumes at lines 2718, 2732 (strict-term, rli<=L cases). Similar structure but with different index relationships. Line 2718: k < rli, leader.log.len() > k but wrong term. Line 2732: k >= rli, leader's log may not reach k.
