...
- 76 transpiler integration tests pass (including 10 verifying generated module public APIs, 1 verus2tla roundtrip for all 7 protocols, 10 D4 pipeline regression tests, 9 message generation per-protocol tests, 9 marshalling round-trip tests, 10 LNext scheduler analysis tests)
...
**Active work**: Phase 17 — Runnable Protocols. Phases 17.1 (transpiler coverage), 17.2 (generic framework), 17.3.1 (message generation), 17.4.1 (LNext analysis), 17.5 (per-protocol implementations), 17.6 (C# runtime integration), and 17.7.2 marshalling round-trip tests are COMPLETE. All 10 protocols can be launched via `dotnet IronProtocolServer.dll <service> <private> protocol=<name>`. Remaining: 17.3.2-3 (RSL marshalling), 17.4.2-4 (scheduler generation+scaffold), 17.6.3 (integration test harness), 17.7.2 (host init tests).
...
**17.4.2: Generate `[scheduler]` TOML config from LNext** (~100 LOC)
- [ ] Auto-populate `[scheduler]` section in protocol TOML files from `analyze-lnext` output
- [ ] Include action-to-message mapping hints (match existential param names to message fields)
- [ ] Add `message_driven` vs `timer_driven` classification heuristic
