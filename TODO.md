...
**Active work**: Phase 17 — Runnable Protocols. Phases 17.1 (transpiler coverage), 17.2 (generic framework), 17.5 (per-protocol implementations), and 17.6 (C# runtime integration) are COMPLETE. All 10 protocols can be launched via `dotnet IronProtocolServer.dll <service> <private> protocol=<name>`. Remaining: 17.3 (transpiler-generated marshalling), 17.4 (transpiler-generated scheduler), 17.6.3 (integration test harness), 17.7 (regression tests).
