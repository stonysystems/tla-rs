...
✅ **571 verified, 0 errors** (with 10 uniform packet-identity assumes in replica_gen.rs + 12 targeted assume() for View-mapping gaps). All `assume(false)` eliminated from generated RSL code.
...
- [ ] **23.8.1.3**: `CRemoveExecutedRequestBatch` — fold over batch → for loop.
  ...
  Transpiler should emit: `for i in 0..batch.len() { result = CRemoveAllSatisfied(&result, &batch[i]) }`.
  Proof: depends on 23.8.1.2's ensures; may need `external_body` lemma for composition.
