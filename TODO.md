...
- [x] **38.8.2**: Second milestone: source-DPOR style backtrack/source-set insertion with exact verdict parity against the baseline on the small cases. **DONE**: 10/20 baseline pass, 9/10 DPOR exact parity. All sub-tasks b-h complete. 38.8.2.a (raise pass floor) remains open — 7 cases blocked on domain expansion (4) or degenerate translation (3).
  - [ ] **38.8.2.a**: Raise the small-case baseline/parity subset above the current 10/20 pass floor. Current blocker summary: 4 cases blocked on domain expansion limit (06 TicketLock, 10 BakeryMutex, 12 DiningPhilosophers, 17 Paxos — all have Map fields causing combinatorial explosion), 3 cases blocked on degenerate translation (14-16 — generated TLA specs use `arbitrary<T>()` placeholders). 20/20 translation success, 0 translation_failed.
