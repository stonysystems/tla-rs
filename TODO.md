        ...
        - [ ] **31.9.4.7.c**: Remove `external_body` from `lemma_2aMessagesFromSameBallotAndOperationMatchWithoutLossOfGenerality` and re-run focused checks for both message2a lemmas. [2026-03-12] Scope check: proof edits are still <500 LOC, but direct body verification at rlimit 40 remains solver-bounded. Focused failing command:
          ...
          - [ ] **31.9.4.7.c.3**: Remove `#[verifier(external_body)]` from `lemma_2aMessagesFromSameBallotAndOperationMatchWithoutLossOfGenerality` and pass focused checks at `--rlimit 40` for both message2a lemmas (`*lemma_Find2aThatCausedVote*` and `*lemma_2aMessagesFromSameBallotAndOperationMatchWithoutLossOfGenerality*`). [2026-03-12] After c.2, direct no-external attempt remains solver-bounded at rlimit 40:
            ...
            - [x] **31.9.4.7.c.3.a**: Extract the old-packet/new-packet contradiction branch into a dedicated helper so the main lemma only dispatches on already-old vs newly-sent cases, reducing context size in the target body. [2026-03-12] Added helper `lemma_2a_old_packet_new_packet_same_ballot_opn_is_impossible` and replaced inline contradiction body with a single helper call. Focused helper check:
              ...
              ⇒ `1 verified, 0 errors`.
            - [ ] **31.9.4.7.c.3.b**: Further reduce target-body load by splitting the both-new-packets same-step branch into its own helper (or equivalent branch-local helperization) and re-check no-external body at `--rlimit 40`.
            - [ ] **31.9.4.7.c.3.c**: Remove `#[verifier(external_body)]` and pass focused checks at `--rlimit 40` for both message2a lemmas.
