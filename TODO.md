...
- [x] **38.8.2**: Second milestone: source-DPOR style backtrack/source-set insertion with exact verdict parity against the baseline on the small cases. **DONE**: 13/20 baseline pass (9 ok + 3 inv_viol + 1 deadlock), 10/12 DPOR exact parity. Protocol cases: TwoPhase + Raft pass. All sub-tasks b-h complete. 38.8.2.a remains open — 7 cases blocked on domain/existential expansion.
  - [ ] **38.8.2.a**: Raise the small-case baseline/parity subset above the current 15/20 pass floor. Remaining 5 cases (10, 14-16, 19) all blocked on verus2tla converter producing degenerate spec bodies. Decomposed sub-tasks below.
    - [x] **38.8.2.a.1**: **OBSOLETE** — Paxos and PBFT now pass via `[collections]` config fix (commits `e8d1513`, `1ac9282`). Init-template extraction no longer needed for these cases.
    - [ ] **38.8.2.a.2**: **Demand-driven successor expansion** (~300 LOC). Would help case 10 (BakeryMutex) where LPickNumber/LMaxNumber use closures the evaluator can't handle. Low priority since case 10 is also blocked on degenerate spec bodies for the other verus2tla-generated cases.
    - [x] **38.8.2.a.3**: **DONE** — existential dedup fix (commit `9927ad2`) + `[collections]` section fix (commits `e8d1513`, `7b2d57b`) resolved the existential expansion issue. Cases 14-16, 19 now fail on degenerate spec bodies, not existential expansion.
    - [ ] **38.8.2.a.4**: **verus2tla Seq<int> elimination + degenerate spec body fix** (>500 LOC). The proper fix for all 5 remaining cases requires improving the verus2tla converter to: (1) not use `Seq<int>` for sent_packets, (2) not produce `s.s.field` double indirection, (3) not emit `arbitrary()` calls, (4) not hash-encode enum tags. This is a separate project from the DPOR engine.
