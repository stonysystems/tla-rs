...
- [x] **37.2.1.d**: Fixed Verus CI: pinned `VERUS_VERSION` to `0.2026.01.14.88f7396` (last version with `KeysAdditionalSpecFns`/`SetIterAdditionalSpecFns` in vstd). Changed download URL to stable path `release/`. Set Rust toolchain to `1.92.0` (required by this Verus). The generated RSL code depends on these removed APIs; updating to the new Verus API requires transpiler changes tracked separately.
