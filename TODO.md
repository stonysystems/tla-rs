...
**Active work**: Phase 17 — Runnable Protocols. All sub-phases COMPLETE except: 17.3.2-3 (RSL marshalling — deferred, RSL-specific), 17.6.3 (integration test harness — blocked, requires .NET SDK). All 10 protocols can be launched via `dotnet IronProtocolServer.dll <service> <private> protocol=<name>`. 1094 total tests (930 unit + 121 integration + 43 roundtrip), 0 warnings.
