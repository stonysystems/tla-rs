...
- [x] **38.8.2**: Second milestone: source-DPOR style backtrack/source-set insertion with exact verdict parity against the baseline on the small cases. **DONE**: 13/20 baseline pass (9 ok + 3 inv_viol + 1 deadlock), 10/12 DPOR exact parity. Protocol cases: TwoPhase + Raft pass. All sub-tasks b-h complete. 38.8.2.a remains open — 7 cases blocked on domain/existential expansion.
  - [ ] **38.8.2.a**: Raise the small-case baseline/parity subset above the current 13/20 pass floor. Current blocker summary: 3 cases blocked on domain expansion (10 BakeryMutex, 17 Paxos, 18 PBFT — Set<LRecord>/Map cross-products), 4 cases blocked on existential expansion (14-16, 19 — verus2tla-generated specs with nested `exists |...: Seq<int>|`). 20/20 translation success. Recent wins: case 06 TicketLock (int 0..2), case 12 DiningPhilosophers (NumPhil=2), case 20 Raft (Server=2). LInit signature fix (commit `ded3b81`) moved cases 14-16,19 from "wrong signature" to "existential expansion" blocker.
