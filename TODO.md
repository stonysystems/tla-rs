...
✅ **586 verified, 0 errors** (with 10 uniform packet-identity assumes in replica_gen.rs). Count dropped from 627 to 586 because ~41 dead functions were stripped from deprecated impl files in Phase 19.7.
...
- Full codebase: 586 verified, 0 errors (RSL + TwoPhase + Paxos + LeaderElection + Raft + ChainReplication + PrimaryBackup + PBFT + VerticalPaxos + EPaxos) — count dropped from 627 due to dead code removal in Phase 19.7
...
- **Generated RSL code still delegates to manual implementations** — proposer_gen (0/12 delegates, all standalone), replica_gen (20/20 delegates), executor_gen (3 delegates), acceptor_gen (1 delegate). Phase 19.2 COMPLETE, Phase 19.7 partial (dead code stripped).
...
**Active work**: 1394 total tests (1059 unit + 145 integration + 53 tla_examples + 43 roundtrip + 38 roundtrip_test + 19 regression + 14 negative + 12 pipeline_e2e + 11 main + 21 doc-ignored), 586 verified, 0 errors. Phase 19.7 dead code stripping COMPLETE. Remaining: Phase 19.3/19.4/19.6 (executor/acceptor/replica standalone), 10 IO trust boundary assumes.
...
| **acceptor_gen.rs** | 7 | **1 (14%)** | 6 | acceptorimpl.rs (212 LOC, stripped) |
...
| **Total** | **63** | **24 (38%)** | **39 (62%)** | ~1,119 LOC manual (down from ~3,852) |
...
- [x] **19.7.1**: Strip dead code from all impl files (completed 2026-02-20):
  - `acceptorimpl.rs`: 678 → 212 LOC (kept CAcceptorProcess1a + CIsLogTruncationPointValid helpers; removed 6 dead methods)
...
- [ ] **19.7.4** (deferred): Full removal of impl files after 19.3/19.4/19.6 complete
  - Remaining live code preventing full removal:
    - `acceptorimpl.rs`: CAcceptorProcess1a (delegated from acceptor_manual.rs) + CIsLogTruncationPointValid helpers
