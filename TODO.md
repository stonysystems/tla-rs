...
1. **Phase 12: Generate proof code to eliminate assumes** — TOP PRIORITY (deferred items: validity proofs, spec refinement, preconditions, collection helpers, unreachable arms)
2. ~~**Phase 16: End-to-end compile & run testing**~~ ✅ COMPLETE — all 4 directions × all examples compile and verify; status matrix complete
3. ~~**Phase 13: Port `tla+2tlars` branch features to main**~~ ✅ COMPLETE
...
**Active work**: Phase 12 (proof generation) is the remaining top priority. The 10 packet-identity assumes are the irreducible IO trust boundary (would require restructuring the IO architecture to eliminate).
