...
- [ ] **31.9**: Run Verus verification with both modules enabled and confirm 0 errors. After 31.9.1 + import fix: 30 errors at rlimit 40 (down from 36 at rlimit 30). Breakdown: 11 assertion, 9 rlimit, 5 postcondition, 3 precondition, 1 termination. 39 verified. Decomposed below.
  ...
  - [ ] **31.9.3**: Fix remaining 28 errors (10 rlimit, 10 assertion, 5 postcondition, 3 precondition). Key blockers: (a) message2b.rs `recv.msg is RslMessage2a` needs Z3 case analysis on match dispatch — may need helper lemma; (b) message1b.rs `pkts.contains(p_2b)` logic issues in proof branches; (c) learner_state.rs postcondition + rlimit; (d) Z3 non-determinism at higher rlimits.
