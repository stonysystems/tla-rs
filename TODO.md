...
1. **Phase 31: RSL Refinement Proof — fix remaining 23 verification errors** — compilation fixed, modules uncommented, 48 verified / 23 errors remaining. Strategy: mark failing fns with `external_body`, then remove bottom-up one at a time using fine-grained verification (`--verify-only-module` / `--verify-function`). Reference: IronFleet Dafny lemmas at https://github.com/microsoft/Ironclad/tree/main/ironfleet. See [Phase 31](#phase-31-rsl-refinement-proof--eliminate-external_body-proof-functions--incomplete-not-verified).
