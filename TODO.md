...
- **Standalone DPOR-based checker prototype is still incomplete** — `transpiler/DPOR_based_model_tla_rs_checker/` now exists and the checked-in machine-readable suite report (`tests/reports/latest.json`) is at **16/20** baseline pass, but `latest.md`, `hard_case_blocker_ledger.md`, and parts of Phase 38 have drifted behind that reality. The remaining pass-count blockers are cases **14-16 and 19**, and the first blocker is no longer domain explosion on 17-18; it is degenerate `translate-tla` output for the generated protocol corpus (`arbitrary()` in `LInit`/actions, wrong `LInit` state typing, flattened `int` record fields, hashed symbolic atoms).

**Next steps (priority order):**
1. **Phase 38: DPOR-Based Model Checker Prototype Track for tla-rs** — finish the remaining four blocked protocol cases by fixing `translate-tla` on the generated corpus shapes (14-16, 19), regenerating the corpus, and resyncing `latest.json`/`latest.md`/the blocker ledger before spending more time on deeper DPOR work. This remains a separate incubator track; do not let it silently rewrite the current mainline checker before the prototype earns it. See [Phase 38](#phase-38-dpor-based-model-checker-prototype-track-for-tla-rs--top-priority).
...
- [x] **38.9.1**: All 3 mandatory protocol gates are now concrete. Cases 17 and 20 pass under the checked-in bounds; case 16 remains blocked on degenerate translated corpus.
  - `16_primarybackup_small`
  - `17_paxos_small`
  - `20_raft_small`
  These three are mandatory because the user explicitly called them out.
  - [x] **38.9.1.a**: **DONE** — `16_primarybackup_small` is now narrowed to the real blocker family: the translated corpus is degenerate before DPOR ever gets a fair shot. Current checked-in `Primarybackup.rs` still has malformed `LInit`, `arbitrary()`-filled action bodies, and hash-encoded symbolic atoms (`6049598361int`, `1048442360int`). Documented in `tests/reports/hard_case_blocker_ledger.md`.
  - [x] **38.9.1.b**: **DONE** — `17_paxos_small` is now **PASS** under the checked-in narrow bounds (`int 0..1`, `max_set_len = 1`). This case is no longer a pass-count blocker and should not be reopened unless bounds are intentionally widened or semantic parity is under investigation.
...
- [x] **38.9.3**: Blocker ledger created, concrete surfaces documented, category updates practiced.
  ...
  - [x] **38.9.3.b**: **SATISFIED** — every remaining blocker entry in `hard_case_blocker_ledger.md` now points to a concrete `translate-tla` surface (`transpiler/src/tla/translator.rs` for state-param inference, `arbitrary()` normalization, symbolic-atom handling, and record-field typing) or to the manifest masking on case 19. Each entry has "Blocker Surface" and "Next Task" columns.
  - [x] **38.9.3.c**: **PRACTICED** — the blocker ledger must be updated in the same commit that changes case status. This resync reclassifies 17-18 as PASS and 14-16/19 as a single translate-tla degeneration family instead of the older existential/domain-explosion story.
- [x] **38.9.4**: Best-known results tracked, bound-shrinking documented, promotion policy in place.
  - [x] **38.9.4.a**: **DONE**. Best-known outcomes recorded in `latest.json` (machine-readable) and `manifest.toml` (expected_primary_result + notes). Current checked-in results are: cases 01-13 PASS, cases 14-16 `checker_error` on degenerate translated corpus, cases 17-18 PASS, case 19 still masked as `known_unimplemented`, and case 20 PASS. All per-case model configs are checked in.
