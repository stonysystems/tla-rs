...
#[test]
fn test_model_check_verticalpaxos_blocker_state_expansion_limit_is_reproducible() {
    let transpiler_bin = resolve_transpiler_binary_for_integration();
    ...
    assert!(
        !output.status.success(),
        "VerticalPaxos model-check should fail until candidate expansion is pruned/bounded more precisely."
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
...
fn test_model_check_raft_blocker_missing_u64_domain_is_reproducible() {
    ...
    assert!(
        !output.status.success(),
        "Raft model-check should fail until named type domains are provided."
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
