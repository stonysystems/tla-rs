...
fn test_rsl_types_transpile_toml_keeps_manual_helpers_binding() {
    ...
    assert!(
        ...
        "manual helper file should include remaining RSL type infrastructure symbols"
    );
    for removed_symbol in [
        ...
        assert!(
            !helper_source.contains(removed_symbol),
            "manual helper file should no longer include `{}` after block-B re-home",
            removed_symbol
        );
    }
    assert!(
        !helper_source.contains("pub fn unreachable_value<T>()"),
        "unreachable_value helper should be generated by type generator, not kept in manual helper file"
    );
    assert!(
        !helper_source.contains("pub type CRslIo = LIoOp<EndPoint, CMessage>;"),
        "CRslIo alias should be generated by type generator, not kept in manual helper file"
    );
}
...
fn test_generated_types_module_public_api() {
    ...
    assert!(
        valid_count >= 2,
        "types_gen.rs should have >= 2 valid() definitions, found {}",
        valid_count
    );
    ...
    assert!(
        view_count >= 2,
        "types_gen.rs should have >= 2 View implementations, found {}",
        view_count
    );
}
