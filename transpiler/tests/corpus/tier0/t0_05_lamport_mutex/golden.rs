...
//!
//! - `req`, `send_seq` and `recv_seq` are `Map<int, int>`: the source declares
//!   them `[Proc -> [Proc -> Nat]]`, and projection removes the *outer* index
//!   (the node) while keeping the inner one (the peer). What remains is this
//!   node's own table about its peers.
//! - `s.req[c.node_id]` is the source's `req[p][p]`: the inner index can be the
//!   acting node itself, and is then written as this node's identity.
//! - `LAdvanceAll`, `LAdvanceOne` and `Lbeats` are the source's own helpers,
//!   kept as functions rather than inlined so the two files line up.
//! - `LMessage::Ack { seq }` carries no `clock`: the source's `AckMessage` sets
//!   `clock |-> 0` only because every message shares one record type, and a
//!   literal carries no information.
...
verus! {
    ...
    pub open spec fn LAdvanceAll(s: LState, c: LConstants) -> Map<int, int> {
        Map::new(c.proc, |d: int| if d == c.node_id { s.send_seq[d] } else { s.send_seq[d] + 1 })
    }
    ...
    pub open spec fn Lbeats(s: LState, c: LConstants, q: int) -> bool {
        s.req[q] == 0 || s.req[c.node_id] < s.req[q] || s.req[c.node_id] == s.req[q] && c.node_id < q
    }
    ...
    pub open spec fn LRequest(
        ...
        &&& sent_packets == c.proc.remove(c.node_id).map(|d: int| LPacket { dst: d, msg: LMessage::Req { clock: s.clock, seq: s.send_seq[d] } })
    ...
    pub open spec fn LReceiveRequest(
        ...
        &&& sent_packets == set![LPacket { dst: src, msg: LMessage::Ack { seq: s.send_seq[src] } }]
    ...
    pub open spec fn LExit(
        ...
        &&& s.crit
        &&& s_.crit == false
        &&& sent_packets == c.proc.remove(c.node_id).map(|d: int| LPacket { dst: d, msg: LMessage::Rel { seq: s.send_seq[d] } })
        &&& s_.send_seq == LAdvanceAll(s, c)
    ...
    pub open spec fn LHandleMessage(
        ...
        match msg {
            ...
            LMessage::Ack { seq } =>
                LReceiveAck(s, s_, c, src, seq, sent_packets),
            LMessage::Rel { seq } =>
                LReceiveRelease(s, s_, c, src, seq, sent_packets),
        }
    }
