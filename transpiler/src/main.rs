...
enum Commands {
    ...
    /// Rewrite one import in transpiler-generated Rust.
    ///
    /// This is intended for mechanical Verus/vstd API migrations when a generated
    /// module cannot yet be regenerated byte-for-byte. The command refuses files
    /// without the transpiler's generated-file marker.
    MigrateGeneratedImport {
        /// Generated Rust file to update in place
        #[arg(long)]
        input: PathBuf,

        /// Existing import path, without `use` or the trailing semicolon
        #[arg(long)]
        from: String,

        /// Replacement import path; omit to remove the import
        #[arg(long)]
        to: Option<String>,

        /// Required number of exact imports to rewrite
        #[arg(long, default_value_t = 1)]
        expect: usize,
    },

    /// Replace an exact text fragment in transpiler-generated Rust.
    ///
    /// This is the guarded fallback for mechanical Verus/vstd API migrations
    /// that are not limited to imports. The command refuses files without the
    /// transpiler's generated-file marker and requires an exact match count.
    MigrateGeneratedText {
        /// Generated Rust file to update in place
        #[arg(long)]
        input: PathBuf,
        ...
        /// Required number of exact fragments to rewrite
        #[arg(long, default_value_t = 1)]
        expect: usize,
    },
...
fn require_generated_file_marker(source: &str) -> Result<()> {
    if !source
        .lines()
        .take(5)
        .any(|line| line.trim() == "// Auto-generated by verus-transpiler")
    {
        return Err(miette::miette!(
            "Refusing to rewrite a file without the verus-transpiler generated-file marker."
        ));
    }
...
fn migrate_generated_import(source: &str, from: &str, to: Option<&str>) -> Result<(String, usize)> {
    require_generated_file_marker(source)?;

    let old_import = format!("use {};", from);
    let new_import = to.map(|path| format!("use {};", path));
    ...
    for segment in source.split_inclusive('\n') {
        ...
        if line_without_cr.trim() == old_import {
            replacements += 1;
            if let Some(ref replacement) = new_import {
                let indent_len = line_without_cr.len() - line_without_cr.trim_start().len();
                rewritten.push_str(&line_without_cr[..indent_len]);
...
fn migrate_generated_text(source: &str, from: &str, to: &str) -> Result<(String, usize)> {
    require_generated_file_marker(source)?;
    if from.is_empty() {
        return Err(miette::miette!(
            "Refusing a generated-text migration with an empty source fragment."
        ));
    }

    let replacements = source.match_indices(from).count();
    Ok((source.replace(from, to), replacements))
}
...
fn handle_command(command: &Commands, cli: &Cli) -> Result<()> {
    match command {
        ...
        Commands::MigrateGeneratedImport {
            input,
            from,
            to,
            expect,
            ...
            let source = std::fs::read_to_string(input).map_err(|e| {
                miette::miette!("Failed to read generated file `{}`: {}", input.display(), e)
            })?;
            let (rewritten, replacements) = migrate_generated_import(&source, from, to.as_deref())?;
            if replacements != *expect {
                return Err(miette::miette!(
                    "Expected {} exact import(s) `{}`, found {} in `{}`; file was not changed.",
                    expect,
                    from,
                    replacements,
                    input.display()
            ...
            std::fs::write(input, rewritten).map_err(|e| {
                miette::miette!(
            ...
            println!(
                "Migrated {} generated import(s) in {}",
                replacements,
                input.display()
            );
            Ok(())
        }
        Commands::MigrateGeneratedText {
            input,
            from,
            to,
            expect,
            ...
            let (rewritten, replacements) = migrate_generated_text(&source, from, to)?;
            if replacements != *expect {
                return Err(miette::miette!(
                    "Expected {} exact text fragment(s), found {} in `{}`; file was not changed.",
                    expect,
                    replacements,
                    input.display()
                ));
            ...
            println!(
                "Migrated {} generated text fragment(s) in {}",
                replacements,
                input.display()
            );
            Ok(())
...
"#,
    ...
    #[test]
    fn test_migrate_generated_import_cli_parsing() {
        let cli = Cli::parse_from([
            "verus-transpile",
            "migrate-generated-import",
            "--input",
            "src/generated/RSL/acceptor_gen.rs",
            "--from",
            "vstd::old::Trait",
            "--to",
            "vstd::new::Trait",
        ...
        match cli.command {
            Some(Commands::MigrateGeneratedImport {
                input,
                from,
                to,
                expect,
            }) => {
                assert_eq!(input, PathBuf::from("src/generated/RSL/acceptor_gen.rs"));
                assert_eq!(from, "vstd::old::Trait");
                assert_eq!(to.as_deref(), Some("vstd::new::Trait"));
                assert_eq!(expect, 2);
            }
            _ => panic!("Expected MigrateGeneratedImport command"),
        }
    }

    #[test]
    fn test_migrate_generated_import_removes_exact_import_only() {
        let source = concat!(
        ...
        let (rewritten, count) =
            migrate_generated_import(source, "vstd::old::Trait", None).unwrap();
        assert_eq!(count, 1);
        assert_eq!(
            rewritten,
            concat!(
    ...
    #[test]
    fn test_migrate_generated_import_rejects_handwritten_source() {
        let err = migrate_generated_import("use vstd::old::Trait;\n", "vstd::old::Trait", None)
            .unwrap_err();
        assert!(err.to_string().contains("generated-file marker"));
    }

    #[test]
    fn test_migrate_generated_text_cli_parsing() {
        let cli = Cli::parse_from([
            "verus-transpile",
            "migrate-generated-text",
            "--input",
            "src/generated/RSL/proposer_gen.rs",
            "--from",
            "Map::old",
            "--to",
            "Map::new",
        ...
        match cli.command {
            Some(Commands::MigrateGeneratedText {
                input,
                from,
                to,
                expect,
            }) => {
                assert_eq!(input, PathBuf::from("src/generated/RSL/proposer_gen.rs"));
                assert_eq!(from, "Map::old");
                assert_eq!(to, "Map::new");
                assert_eq!(expect, 2);
            }
            _ => panic!("Expected MigrateGeneratedText command"),
        }
    }

    #[test]
    fn test_migrate_generated_text_replaces_exact_fragment() {
        let source = concat!(
            "// Auto-generated by verus-transpiler\n",
            "// DO NOT EDIT MANUALLY\n\n",
            "Map::old(domain, value)\n",
            "// Map::old should remain\n",
        );
        let (rewritten, count) =
            migrate_generated_text(source, "Map::old(domain, value)", "Map::new(domain, value)")
                .unwrap();
        assert_eq!(count, 1);
        assert!(rewritten.contains("Map::new(domain, value)"));
        assert!(rewritten.contains("// Map::old should remain"));
    }

    #[test]
    fn test_migrate_generated_text_rejects_empty_source_fragment() {
        let source = "// Auto-generated by verus-transpiler\n";
        let err = migrate_generated_text(source, "", "anything").unwrap_err();
        assert!(err.to_string().contains("empty source fragment"));
    }
