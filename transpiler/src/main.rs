...
fn execute_model_check(
    ...
    /// Phase 38.17.2: Substitute formal parameters with actual arguments in an expression.
    fn substitute_call_args(
        body: &verus_transpiler::ast::Expr,
        params: &[verus_transpiler::ast::Parameter],
        args: &[verus_transpiler::ast::Expr],
    ) -> verus_transpiler::ast::Expr {
        use verus_transpiler::ast::Expr;
        // Build a substitution map: param_name → arg_expr
    ...
    fn substitute_expr(
        expr: &verus_transpiler::ast::Expr,
        subst: &std::collections::BTreeMap<String, &verus_transpiler::ast::Expr>,
    ) -> verus_transpiler::ast::Expr {
        use verus_transpiler::ast::Expr;
        match expr {
            Expr::Ident(name) => {
            ...
            // Literals, paths, empty collections, etc. — no identifiers to substitute
            _ => expr.clone(),
        }
    }
    ...
    // Fix: look up the called function, substitute arguments, and expand
    // the conjunction into individual normalized constraints.
    {
        use verus_transpiler::modelcheck::ir::{
            normalize_constraint_pub, BranchConstraintIr, ConstraintRoot,
        };
        let lnext = &bundle.entrypoints.lnext;
        let current_state_param = lnext.params[0].name.as_str();
        let next_state_param = lnext.params[1].name.as_str();
        let constants_param = lnext.params.get(2).map(|p| p.name.as_str());
        ...
        for branch in &mut transition.branches {
            ...
            for constraint in &branch.constraints {
                if let BranchConstraintIr::Predicate { expr } = constraint {
                    if let verus_transpiler::ast::Expr::Call { func, args } = expr {
                        {
