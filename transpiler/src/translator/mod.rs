...
impl ProofNeeds {
    ...
    fn scan_expr(&mut self, expr: &ExecExpr) {
        match expr {
            ...
            ExecExpr::Block(stmts) => {
                ...
                //   __X.remove(element)
                self.scan_block_for_remove_sites(stmts);
                for stmt in stmts {
                    self.scan_expr(stmt);
                }
            }
    ...
    /// Scan consecutive statement pairs in a Block to detect remove sites.
    ///
    /// Looks for the pattern generated by `extract_set_mutations_from_struct`:
    /// ```ignore
    /// let mut __field = clone_hashset(&s.field);
    /// __field.remove(element);
    /// ```
    /// Records `RemoveSite { source_view: "s.field@", element: "*element" or "element" }`.
    fn scan_block_for_remove_sites(&mut self, stmts: &[ExecExpr]) {
        ...
        for stmt in stmts {
            ...
            if let ExecExpr::Let { pattern, value, .. } = stmt {
                if let Some(tmp_name) = pattern.strip_prefix("mut ") {
                    if let ExecExpr::Call { func, args } = value.as_ref() {
                        if func == "clone_hashset" && args.len() == 1 {
