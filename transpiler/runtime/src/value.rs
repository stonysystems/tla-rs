...
/// Runtime error for value construction violations (e.g. collection bounds,
/// duplicate fields). Deliberately minimal — the main transpiler maps this
/// to its richer `TranspileError::Config` via `From`.
#[derive(Debug, Clone)]
...
/// Result type for runtime value operations.
...
#[derive(Debug)]
pub struct FingerprintCache(std::sync::atomic::AtomicU64);
...
impl RuntimeValue {
    ...
    pub fn struct_value<I>(ty: impl Into<String>, fields: I) -> RuntimeResult<Self>
    where
        I: IntoIterator<Item = (String, RuntimeValue)>,
    {
        Ok(Self::Struct {
    ...
    pub fn set_bounded<I>(values: I, bounds: &RuntimeCollectionBounds) -> RuntimeResult<Self>
    where
    ...
    pub fn map_bounded<I>(entries: I, bounds: &RuntimeCollectionBounds) -> RuntimeResult<Self>
    where
...
fn collect_named_fields<I>(fields: I) -> RuntimeResult<NamedFields>
where
    I: IntoIterator<Item = (String, RuntimeValue)>,
{
...
use crate::small_int_set::SmallIntSet;
...
pub enum SetReprIter<'a> {
    ...
    SmallInt(crate::small_int_set::SmallIntSetIter),
}
