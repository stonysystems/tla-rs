//! Generated from TLA+ module: Types
//!
//! This file was auto-generated by verus-transpiler.

use vstd::prelude::*;
use vstd::seq::*;
use vstd::set::*;

verus! {

/// Record type for Types module
pub struct LRecord {
    pub accept_senders: int,
    pub ballot: int,
    pub cmd: int,
    pub committed_count: int,
    pub dep_count: int,
    pub executed_count: int,
    pub fast_quorum_size: int,
    pub has_conflict: int,
    pub is_leader: int,
    pub max_resp_seq: int,
    pub my_id: int,
    pub num_replicas: int,
    pub phase: int,
    pub preaccept_senders: int,
    pub quorum_size: int,
    pub seq: int,
}

/// State for Types module
pub struct LState {
}


/// Constants operator
pub open spec fn LConstants() -> { my_id: Set<int>, quorum_size: Set<int>, fast_quorum_size: Set<int>, num_replicas: Set<int> } {
    LRecord { accept_senders: 0int, ballot: 0int, cmd: 0int, committed_count: 0int, dep_count: 0int, executed_count: 0int, fast_quorum_size: int, has_conflict: 0int, is_leader: 0int, max_resp_seq: 0int, my_id: int, num_replicas: int, phase: 0int, preaccept_senders: 0int, quorum_size: int, seq: 0int }
}

/// State operator
pub open spec fn LState() -> { committed_count: Set<int>, ballot: Set<int>, executed_count: Set<int>, phase: int, dep_count: Set<int>, is_leader: Set<bool>, accept_senders: Set<Set<int>>, has_conflict: Set<bool>, cmd: Set<int>, seq: Set<int>, preaccept_senders: Set<Set<int>>, max_resp_seq: Set<int> } {
    LRecord { accept_senders: int.powerset(), ballot: int, cmd: int, committed_count: int, dep_count: int, executed_count: int, fast_quorum_size: 0int, has_conflict: bool, is_leader: bool, max_resp_seq: int, my_id: 0int, num_replicas: 0int, phase: LInstancePhase(), preaccept_senders: int.powerset(), quorum_size: 0int, seq: int }
}

/// InstancePhase operator
pub open spec fn LInstancePhase() -> Set<int> {
    set![Empty, PreAccepted, Accepted, Committed, Executed]
}

/// EPaxosMessage operator
pub open spec fn LEPaxosMessage() -> Set<int> {
    set![PreAccept, PreAcceptOk, Accept, AcceptOk, Commit]
}

} // verus!
