...
pub open spec fn LNext(s: LState, c: LConstants, s_: int) -> bool {
    exists |b, sent_packets| (Seq(c.VPMessage).contains(sent_packets)) && (LPrepare(s, c, s, s_, c, b, sent_packets) || exists |prepare_bal, sent_packets| (Seq(c.VPMessage).contains(sent_packets)) && (LSendPromise(s, c, s, s_, c, prepare_bal, sent_packets) || exists |sender, promise_bal, promise_v_bal, promise_val, sent_packets| (Seq(c.VPMessage).contains(sent_packets)) && (LReceivePromise(s, c, s, s_, c, sender, promise_bal, promise_v_bal, promise_val, sent_packets) || exists |b, v, sent_packets| (Seq(c.VPMessage).contains(sent_packets)) && (LAccept(s, c, s, s_, c, b, v, sent_packets) || exists |sender, accept_bal, sent_packets| (Seq(c.VPMessage).contains(sent_packets)) && (LReceiveAccepted(s, c, s, s_, c, sender, accept_bal, sent_packets) || exists |sent_packets| Seq(c.VPMessage).contains(sent_packets) && (LCommit(s, c, s, s_, c, sent_packets) || exists |sent_packets| Seq(c.VPMessage).contains(sent_packets) && (LReconfigure(s, c, s, s_, c, sent_packets) || exists |witness_val, sent_packets| (Seq(c.VPMessage).contains(sent_packets)) && (LWitnessSync(s, c, s, s_, c, witness_val, sent_packets) || exists |new_config, val, sent_packets| (Seq(c.VPMessage).contains(sent_packets)) && (LSync(s, c, s, s_, c, new_config, val, sent_packets) || exists |sent_packets| Seq(c.VPMessage).contains(sent_packets) && LDeactivate(s, c, s, s_, c, sent_packets))))))))))
}
