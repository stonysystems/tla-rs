...
| `PBFT` | `src/protocol/PBFT/pbft.rs`, `src/protocol/PBFT/types.rs` | `transpiler/tests/model_check_fixtures/pbft_state_expansion_limit.model.toml` | `bfs`, exact (`state_dedup=canonical`) | `ok` (best-effort) | `4 / 3 / 1 / ~100` | N/A | `test_model_check_pbft_bounded_run`, `reports/model_check/pbft_small.json` (best-effort: may be placeholder on CI runners where LState expansion exceeds limits) |
...
| `PrimaryBackup` | `src/protocol/PrimaryBackup/primarybackup.rs`, `src/protocol/PrimaryBackup/types.rs` | `transpiler/tests/model_check_fixtures/primarybackup_small.model.toml` | `bfs`, exact (`state_dedup=canonical`) | `ok` | `3 / 3 / 1 / 17` | N/A | `test_model_check_primarybackup_helper_call_branches_bounded_run`, `test_model_check_primarybackup_real_safety_invariants_bounded_run`, `reports/model_check/primarybackup_small.json`, `reports/model_check/primarybackup_safety_invariants.json` |
...
The `elapsed_ms` column is indicative only and is **not** asserted by
`test_model_check_exact_mode_baseline_snapshot_matches_checked_in_artifacts`: it is wall-clock,
so it differs per host, and pinning it made every honest artifact refresh look like a test
failure (see TODO 37.2.1.i). The structural columns are asserted.
